    ...
    if (result.status === 'ok') {
      ...
      if (data) {
        defaultData.serviceNum = [
          ...
          {
            title: '学生总数',
            num: Number((data?.khfwxss || 0) + (data?.xs_count || 0))
          },
          {
            title: '收款总额',
          ...
          {
            title: '退款总额',
            num: Number((data?.tk_count || 0) + (data?.zztk_amount || 0) + (data?.khtk_amount || 0)).toFixed(2)
          }
        ];
        ...
        defaultData.enrollNum = data.xxbm?.length
          ? [].map.call(data.xxbm, (item: any) => {
              return {
                school: item.XXMC,
                value: Number(item.count + item.khfwxs_count)
              };
            })
          : [];
      }
