...
const List = (props: { type: string, data?: any, noDataImg?: any, noDataText?: string ,detailsPath?: string }) => {
  ...
  return (
    <div className={styles.annceList}>
      {data?.length ? <ul>
        {data.map((item: { BT?: string, updatedAt?: string ,RQ?: string,SFTT?: number, KCMC?: string, KHJYJG?: any, QYMC?: string }) => {
          return <li key={item.BT}>
            ...
            >
              ...
              <span>{item.BT || item.KCMC || item.QYMC}{'\u00A0\u00A0\u00A0'}{item.KHJYJG ? <div className={styles.Organization}>{item.KHJYJG.QYMC}</div> : <></>}</span>
